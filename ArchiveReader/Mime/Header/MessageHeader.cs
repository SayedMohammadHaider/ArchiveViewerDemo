	...
	public sealed class MessageHeader
	{
		...
		private void ParseHeader(string headerName, string headerValue)
		{
			...
			switch (headerName.ToUpperInvariant())
			{
				...
				case "X-NOTES-ITEM":
					var xnotestemp = headerValue.Split(',');

					foreach (var keyword in xnotestemp)
					{
						string[] xnotestemp2 = keyword.Split(';');
						if (xnotestemp2.Length > 1)
						{
							string fieldname = xnotestemp2[1].Substring(xnotestemp2[1].IndexOf('=') + 1);
							string fieldvalue = xnotestemp2[0];
							NotesItems.Add(fieldname,fieldvalue);
						}
